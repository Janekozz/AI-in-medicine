...
dxEpsi = imfilter(phi,Hx); dxEpsi(outerMask==1)=0; % To ensure that the stream line stops at the end of the mask, not at the begining of the outer mask
...
% Calculate and visualize the stram lines from the source boundary to the destination one
...
% Curvature vectors
plot([Vertices(:,1) Vertices(:,1)+ks.*N(:,1)]', [Vertices(:,2) Vertices(:,2)+ks.*N(:,2)]', ...
'g', 'LineWidth', 1.5);
...
tourImg = calculateTortuosity2D(pointsSorted, geodesicDist, size(geoDistance), tstDistance);
...
title(['Tortuosity Overlay (' boundaryType ' boundary)'], 'FontSize', 13, 'FontWeight', 'bold');
hold off
